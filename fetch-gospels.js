...
function httpGet(url, hops=5) {
  return new Promise((resolve, reject) => {
    ...
    const req = lib.get(url, {timeout:20000,
      headers:{'User-Agent':'Mozilla/5.0 (compatible; GospelBot/2.0)'}
    }, res => {
      ...
      res.on('data',c=>d+=c); res.on('end',()=>resolve(d.trim()));
    });
    req.on('error', reject);
...
// Ambrosian: try vangelodelgiorno.org with date-specific URL
async function fetchAmbrosiano(d) {
  ...
  for (const url of urls) {
    try {
      ...
      // Extract reference (look for "Dal Vangelo..." pattern)
      let ref = '';
      const rm = html.match(/Dal Vangelo[^<"]{5,80}/i);
      if (rm) ref = clean(rm[0]);

      // Extract main content block — try multiple selectors via regex
      const blocks = [
        html.match(/<div[^>]+class="[^"]*entry-content[^"]*"[^>]*>([\s\S]{200,5000}?)<\/div>/i),
        html.match(/<div[^>]+class="[^"]*post-content[^"]*"[^>]*>([\s\S]{200,5000}?)<\/div>/i),
        html.match(/<article[^>]*>([\s\S]{200,8000}?)<\/article>/i),
      ];

      for (const m of blocks) {
        if (!m) continue;
        const lines = clean(m[1]).split('\n').filter(l=>l.length>25).slice(0,20);
        if (lines.length >= 2) {
          return { reference: ref || 'Vangelo Ambrosiano', text: lines.join('\n') };
        }
      }
    } catch(_) {}
  }
...
async function main() {
    ...
    try {
      ...
      gospels[iso] = {
        ...
        ambrosiano: ambrosiano
          ? { ...ambrosiano, commentTitle:romano.commentTitle,
              commentAuthor:romano.commentAuthor, commentText:romano.commentText }
          : { ...romano, reference: romano.reference + ' (rito romano)' },
      };
  ...
  console.log(`\n\n✅  Nuovi:${dl}  Saltati:${sk}  Errori:${err}  Totale:${Object.keys(sorted).length} (${kb}KB)\n`);
}
